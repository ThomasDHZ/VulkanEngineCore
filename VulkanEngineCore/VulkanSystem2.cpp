...
uint32					 VulkanSystem2::SwapChainImageCount()	const { return m_swapChain.ImageIndex(); }
VkExtent2D				 VulkanSystem2::SwapChainResolution()	const { return m_swapChain.SwapChainResolution(); }
ivec2					 VulkanSystem2::RenderPassResolution()	const { return m_swapChain.RenderPassResolution(); }
VkCommandPool            VulkanSystem2::CommandPool()			const { return m_commandBuffer.CommandPool(); }
Vector<VkCommandBuffer>  VulkanSystem2::CommandBufferList()	    const { return m_commandBuffer.CommandBufferList(); }